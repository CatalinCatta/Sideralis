...
namespace Mkey
{
    public class ParallaxPlane : MonoBehaviour
    {
        ...
        public void UpdateInfinitePlane(float cameraPosX)
        {
            ...
            if (cameraPosX > mainContainerPosition.x + halfMapSizeX ||
                cameraPosX < mainContainerPosition.x - halfMapSizeX)
            {
        ...
        public void UpdateInfinitePlane(Vector2 cameraPos)
        {
            if (!canUpdate) return;
            mainContainerPosition = mainContainer.position;
            var dPos = cameraPos - (Vector2)mainContainerPosition;
            ...
            additContainer_2.position = new Vector3(additContainer.position.x, additContainer_1.position.y,
                additContainer_1.position.z);
            ...
            var outy = false;
            tempContainer = mainContainer;
            if (cameraPos.x > mainContainerPosition.x + halfMapSizeX ||
                cameraPos.x < mainContainerPosition.x - halfMapSizeX) outx = true;

            if (cameraPos.y > mainContainerPosition.y + halfMapSizeY ||
                cameraPos.y < mainContainerPosition.y - halfMapSizeY) outy = true;
